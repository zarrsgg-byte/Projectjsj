...
hasAnyTaskCompleted: boolean = false; // متغير لتتبع إذا اكتملت أي مهمة
...
async start(): Promise<boolean> {  
    ...
    this.started = true;  
    this.hasAnyTaskCompleted = false; // إعادة تعيين عند بدء كويست جديد  
    this.quests.clear();  
    ...
    if (this.process) {  
        if (this.quests.size > 0) {  
            this.quests.forEach(q => {  
                this.send({ type: "kill", target: q.id });  
            });  
        } else if (this.selectedQuest) {  
            this.send({ type: "kill", target: this.selectedQuest.id });  
        }  
    }  
...
// دالة للتحقق مما إذا كانت أي مهمة قد اكتملت  
private checkAnyTaskCompleted(): boolean {  
    const quest = this.selectedQuest;  
    if (!quest || !quest.progress) return false;  

    // التحقق من وجود أي مهمة مكتملة  
    return quest.progress.some(task => task.completed === true);  
}  
...
async updateProgress(progress: number, completed: boolean): Promise<void> {  
    ...
    // بعد تحديث التقدم، تحقق مما إذا كانت أي مهمة قد اكتملت  
    this.hasAnyTaskCompleted = this.checkAnyTaskCompleted();  
}  
...
generateMessage(): any {  
    ...
    if (isAvatarDecorationReward && this.questDecorationBuffer) {  
        ...
        Logger.info(`User ${this.id}: showing avatar_decoration.png`);  
    }  
    ...
    // زر القائمة الخاصة (الصاروخ) - يبقى كما هو
    const rocketButton = new ButtonBuilder()  
        .setCustomId("rocket_menu")  
        .setEmoji("🚀")  
        .setLabel(i18n.t("buttons.rocketMenu") || "Rocket Menu")  
        .setStyle(ButtonStyle.Secondary)  
        .setDisabled(this.started || this.stoped);  
    ...
    // إظهار رسالة تغيير كلمة المرور إذا كانت أي مهمة قد اكتملت  
    if (this.started && this.hasAnyTaskCompleted) {  
    ...
    // الصف الأول: القائمة الرئيسية (Select Menu)
    components.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu));

    // الصف الثاني: الأزرار (جميع الأزرار بما فيها زر الصاروخ)
...
prepareForNextQuest(nextQuest: Quest, remainingQuests: Collection<string, Quest>): void {  
    ...
    this.hasAnyTaskCompleted = false; // إعادة تعيين متغير اكتمال المهام  
...
destroy(): void {  
    ...
    this.questImageBuffer = null;  
    this.showQuestImage = true;  
    this.hasAnyTaskCompleted = false;  
}
