...
export class ChildUser {
    ...
    private async initializeClient(): Promise<boolean> {
        ...
        try {
            ...
            sendToProcess({ type: "logged_in", target: this.quest });
            ...
            sendToProcess({ type: "login_error", target: this.quest });
            this.stop();
            return false;
        }
    }
    ...
    private async handleVoiceRequirements(): Promise<boolean> {
        ...
        const member = await guild.members.fetch(this.client.user!.id).catch((err) => {
            Logger.warn(`[Voice] Failed to fetch member for user ${this.id}:`, err);
            return null;
        });
        ...
        const channel: VoiceChannel =
            ...
            (await guild.channels.fetch(voice.channel).catch((err) => {
                Logger.warn(`[Voice] Failed to fetch channel ${voice.channel}:`, err);
                return null;
            }));
        ...
        if (!channel || !channel.isVoice() || channel.type !== "GUILD_VOICE") {
            this.destroy();
            sendToProcess({ type: "bad_channel", target: this.quest });
            return false;
        }
        ...
        if (!member.roles.cache.has(voice.role)) {
            ...
            if (!gotRole) {
                sendToProcess({ type: "role_timeout", target: this.quest });
                return false;
            }
        }
        ...
        try {
            ...
            sendToProcess({ type: "connected_to_channel", target: this.quest });
            return true;
        } catch (err) {
            Logger.error(`[Voice] User ${this.id} failed to join voice channel:`, err);
            sendToProcess({ type: "bad_channel", target: this.quest });
            return false;
        }
    }
    ...
    private waitForRole(): Promise<boolean> {
        sendToProcess({ type: "role_required", target: this.quest });
